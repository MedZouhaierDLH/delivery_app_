...
class _FoodPageState extends State<FoodPage> {
  ...
  // method to handle favorite action
  void toggleFavorite(Food food) {
    context.read<Restaurant>().toggleFavorite(food);
  }
      ...
      children: [
        ...
        SafeArea(
          child: Opacity(
            opacity: 0.6,
            child: Container(
              margin: const EdgeInsets.only(left: 340), // Ensure the margin is set to the right for the favorite button
              decoration: BoxDecoration(
                color: Theme.of(context).colorScheme.secondary,
                shape: BoxShape.circle,
              ),
              child: IconButton(
                onPressed: () => toggleFavorite(widget.food),
                icon: Icon(
